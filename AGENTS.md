...
- **No app-authored JS beyond what's declaratively necessary.** Mobile nav
  ...
  otherwise) — not custom JS. The Esc-navigation listener, the theme
  toggle, the ⌘K command palette's `fetch()`+innerHTML, and the
  reminder-toast poll (`layout.html`'s `setInterval` hitting
  `/notifications/poll` — see
  [Scheduled jobs & reminders](DOCUMENTATION.md#scheduled-jobs--reminders))
  are the deliberate exceptions, because there's no declarative way to do
  any of them; each is small, single-purpose, and commented inline in
